...
export default function ContactBannerPage() {
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        const response = await fetch('/api/contact/banner')
        if (response.ok) {
        ...
        toast.error('Failed to fetch banner content')
      } finally {
        setIsFetching(false)
      }
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50/50 py-8">
      <div className="w-full max-w-4xl">
        <div className="mb-8 text-center">
          <h1 className="font-serif text-3xl font-bold text-[#4C9296]">Bannière de Contact</h1>
          <p className="mt-2 text-black">
            Personnalisez la bannière de la page contact.
          </p>
        </div>
